(function(a) {
    a.flexslider = function(c, b) {
        ...
        d.init = function() {
            ...
            if (d.vars.controlNav) {
                ...
                //初始化
                if(d.controlNav.length > 7 ){
                    d.controlNav.filter(':gt(2)').parent().hide();
                    d.controlNav.filter(':first').parent().after("<span class='first-icon'>...</span>");
                    d.controlNav.parents('.flex-control-nav').find('.first-icon').hide();
                    d.controlNav.filter(':last').parent().before("<span class='last-icon'>...</span>");
                    d.controlNav.filter(':last').parent().show();
                }            
                ...
                d.controlNav.bind(d.eventType, function(i) {
                    ...
                    if (!a(this).hasClass("active")) {
                        ...
                        //新增分页
                        var srartNum =  parseInt(d.controlNav.index(a(this)) - 2),
                            currentNum = d.controlNav.index(a(this)),
                            endNum = parseInt(d.controlNav.index(a(this)) + 2);


                            if(currentNum > 0){
                                d.controlNav.filter(':lt('+srartNum+')').parent().hide();
                                d.controlNav.filter(':gt('+endNum+')').parent().hide();

                                d.controlNav.eq(srartNum).parent().show();
                                d.controlNav.eq(currentNum-1).parent().show();
                                d.controlNav.eq(currentNum).parent().show();
                                d.controlNav.eq(currentNum+1).parent().show();
                                d.controlNav.eq(endNum).parent().show();

                                d.controlNav.filter(':last').parent().show();
                                d.controlNav.parents('.flex-control-nav').find('.last-icon').show();


                                if( parseInt(d.controlNav.length - currentNum) < 5 ){
                                    d.controlNav.parents('.flex-control-nav').find('.last-icon').hide();
                                } 

                                if( currentNum == 0){
                                    d.controlNav.filter(':first').parent().show();
                                }

                                if(currentNum >= 3){
                                    d.controlNav.filter(':first').parent().show();
                                    d.controlNav.parents('.flex-control-nav').find('.first-icon').show();
                                } else {
                                    d.controlNav.parents('.flex-control-nav').find('.first-icon').hide();
                                }
            ...
            if (d.vars.autoPlay) {
                if (d.vars.pauseOnHover && d.vars.autoPlay) {
                    d.hover(function() {
                        d.pause()
                    }, function() {
                        d.resume()
            ...
            if (d.vars.pausePlay) {
                ...
                var h = (d.vars.autoPlay) ? "pause" : "play";
        ...
        d.flexAnimate = function(f, e) {
            if (!d.animating) {
                ...
                if (d.vars.controlNav) {
                    ...
                    //新增分页
                    d.controlNav.filter(':lt('+(f-2)+')').parent().hide();
                    d.controlNav.filter(':gt('+(f+2)+')').parent().hide();

                    //显示选中前后2个的值
                    d.controlNav.eq(f-2).parent().show();
                    d.controlNav.eq(f-1).parent().show();
                    d.controlNav.eq(f).parent().show();
                    d.controlNav.eq(f+1).parent().show();
                    d.controlNav.eq(f+2).parent().show();
                    d.controlNav.filter(':last').parent().show();

                    if( parseInt(d.controlNav.length - f) < 5 ){
                        d.controlNav.parents('.flex-control-nav').find('.last-icon').hide();
                    } else {
                        d.controlNav.parents('.flex-control-nav').find('.last-icon').show();
                    }
    ...
    a.flexslider.defaults = {
        ...
        keyboardNav: false,       // 是否使用键盘左右键控制
        touchSwipe: true,        // 设置手机触摸
        prevText: "Previous",    //左键按钮文字设置
        nextText: "Next",        //右键按钮文字设置
        pausePlay: false,
        randomize: false,
        slideToStart: 0,        //设置从第几个开始显示
        animationLoop: true,    //是否循环切换
        pauseOnAction: true,
        pauseOnHover: false,
        controlsContainer: "",
        manualControls: "",
