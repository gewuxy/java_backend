...
//取得汉字的拼音
getGB2312Spell:function(str,sp){
    var i,c,t,p,ret="";
    if(sp==null)sp="";
    for(i=0;i<str.length;i++){
		var ch = str.charCodeAt(i);
        if(ch>=0x4e00){
            p=this.strGB.indexOf(str.charAt(i));
            if(p>-1&&p<3755){
                for(t=this.pinyinSpell.length-1;t>0;t=t-2)if(this.pinyinSpell[t]<=p)break;
                if(t>0)ret+=this.pinyinSpell[t-1]+sp;
            }
        }else{//这个else是我自己加上去的,原来没有.
			ret+=String.fromCharCode(ch)+sp;
		}
    }
    return ret.substr(0,ret.length-sp.length);
},
getCharIndexStr:function(str){
	var chars = str.split('');
	var rtn='';
	for(var i=0;i<chars.length;i++){
		var ch=chars[i];
		var convertStr=this.getGB2312Spell(ch);
		rtn+=convertStr.charAt(0);
	}
	return rtn;
},
gt:function(str1,str2){
var str1IndexStr=this.getCharIndexStr(str1);
var str2IndexStr=this.getCharIndexStr(str2);
//alert(str1IndexStr);
//alert(str1IndexStr+"---"+str2IndexStr);
//alert(str1IndexStr>str2IndexStr);
return (str1IndexStr>str2IndexStr);
},
sortOptions:function(selectId){
	var select = document.getElementById(selectId);
	var options = select.options;
	for(var i=0;i<options.length;i++){
		for(var j=0;j<options.length-i-1;j++){
			var preOption=options[j];
			var nextOption=options[j+1];
			var preOptText = preOption.innerHTML;
			var nextOptText = nextOption.innerHTML;
			//alert(preOptText+"---"+nextOptText);
			if(this.gt(preOptText,nextOptText)){
				this.swapNode(preOption,nextOption);
			}
		}
	}
...
function testBubbleSort(){
	var arr=[21,3,4,6,11,33,22,5,8,9];
