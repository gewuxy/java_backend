    ...
    <script>
        ...
        $(function(){
            ...
            $(".btnAdd-metting-faq").click(function(){
                ...
                $('.m-layer-bd').show();
                $("#questionEditFrame").attr("src","${ctx}/func/meet/survey/edit?paperId=${paper.id}");
            });

            $(".bianji").click(function(){
                $('.mask-wrap').addClass('dis-table');
                $('.m-layer-bd').show();
                $("#questionEditFrame").attr("src","${ctx}/func/meet/survey/edit?paperId=${paper.id}&questionId="+$(this).attr("qid"));
            });
            ...
            $(".del").click(function(){
                ...
                top.layer.confirm("确定要删除此试题吗？",function(){
                    $.get('${ctx}/func/meet/survey/del',{'paperId':'${paper.id}','questionId':questionId},function () {
                        top.layer.closeAll();
        ...
        function batchePaper(){
            ...
            $.ajaxFileUpload({
                url: '${ctx}/func/meet/survey/batch?meetId='+meetId+'&moduleId='+moduleId, //用于文件上传的服务器端请求地址
...
<div class="g-main  mettingForm clearfix">
    ...
    <div class="tab-bd" style="padding-bottom:500px;">
        ...
        <div class="metting-info clearfix">
            <p class="rowMargin"><label class="formButton">上传问卷<input type="file" name="file" id="uploadPaper" class="none"></label>&nbsp;&nbsp;&nbsp;&nbsp;点击 <a href="${ctxStatic}/upload/temp/survey_template.xlsx" class="color-blue">问卷模版</a> 并下载，以最后一次上传为准</p>
                ...
                <c:forEach items="${paper.questionList}" var="question" varStatus="status">
                    <div class="add-metting-faq-content" >
                        <div class="movie_box" style="border: 1px solid rgb(255, 255, 255);">
                            ...
                            <div class="wjdc_list">
                                <li><div class="tm_btitlt"><i class="nmb">${status.index+1}</i>. <i class="btwenzi">${question.title}</i></div> </li>&nbsp;&nbsp;
                                <c:choose>
                                    <c:when test="${question.qtype == 0}">
                                        <c:forEach items="${question.optionList}" var="op">
                                            <li><label> <input name="option[${question.id}]" type="radio" ><span>${op.key} . ${op.value}</span></label></li>
                                        </c:forEach>
                                    </c:when>
                                    <c:when test="${question.qtype == 1}">
                                        <c:forEach items="${question.optionList}" var="op">
                                            <li><label> <input name="option[${question.id}]" type="checkbox" value=""><span>${op.key} . ${op.value}</span></label></li>
                                        </c:forEach>
                                    </c:when>
                                    <c:when test="${question.qtype == 2 || question.qtype == 3}">
                                        <textarea name=""  cols="" rows="" class="input_wenbk btwen_text btwen_text_tk" ></textarea>
                                    </c:when>
                                </c:choose>
                            </div>
