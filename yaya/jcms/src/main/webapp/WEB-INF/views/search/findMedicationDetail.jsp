...
<body>
                    ...
                    <div class="col-lg-8 ">
                        <div class="v2-subPage-searchItem">
                            ...
                            <div class="v2-subPage-searchTitle clearfix">
                                <img src="${statics}/images/v2/searchItem-title3.png" alt="">
                            </div>
                            <div class="v2-subPage-serarch-content v2-helpPage-item">
                                ...
                                <span class="pr v2-helpPage-select-a">
                                    ...
                                    <div class="v2-search-form clearfix">
                                        <input name="searchingText" id="searchingText" type="text" placeholder="药品通用名/商品名/批准文号" class="form-text">
                        ...
                        <div class="v2-searchText ">
                            <c:if test="${searchingText!=null}">
                                搜索结果：${searchingText}
                            </c:if>
                            <span class="fr"><a href="${base}/search/pharmacist/advice/list" class="button buttonGreen">咨询药师建议</a></span>
                        </div>
                        <div class="v2-searchTable">
                            <table class="formPriceTable fs14 formGreenHeadStyle" style="width:100%" cellspacing="0">
                                <colgroup>
                                    <col class="col3">
                                    <c:forEach items="${detailKeys}" var="detailKey" varStatus="status">
                                        <col class="col5">
                                    </c:forEach>
                                </colgroup>
                                <thead>
                                    <tr>
                                        <th>&nbsp;通用名</th>
                                        <%
                                            String[]detailKeys={"商品名","规格","批准文号","生产厂家"};
                                            request.setAttribute("detailKeys",detailKeys);
                                        %>
                                        <c:forEach items="${detailKeys}" var="detailKey" varStatus="status">
                                            <th>${detailKey}</th>
                                        </c:forEach>
                                ...
                                <tbody>
                                    <c:forEach items="${dataFileList}" var="dataFile" varStatus="status">
                                        <tr>
                                            <td>
                                                <c:if test="${dataFile.filePath==null}">
                                                    <span title="没有找到说明书哦 0_0">${dataFile.title}</span>
                                                </c:if>
                                                <c:if test="${dataFile.filePath!=null}">
                                                    <a href="${base}/${dataFile.filePath}">${dataFile.title}</a>
                                                </c:if>
                                            </td>
                                            <c:forEach items="${detailKeys}" var="detailKey" varStatus="status">
                                                <th>${dataFileDetailMapMap[dataFile["id"]][detailKey]}</th>
                                            </c:forEach>
                                        </tr>
                                    </c:forEach>
                                </tbody>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            //分页按钮
            var url = "${base}/search/find/medication/detail";//指定跳转路径
