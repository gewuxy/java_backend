...
<div class="warp">
    <div class="header">
        <div class="staus-bar"><strong>填下以下信息</strong></div>
    </div>

    <div class="item">
        <div class="item-area login-input">
            <form action="${ctx}/weixin/register" method="post" id="bindForm" name="bindForm">
                <div class="formrow radius login-input-item">
                    ...
                    <c:if test="${empty pubUserName}">
                        <label for="" class="cells-block pr"><input type="text" class="icon-register-hot" name="invite" value="${invite}" id="invite" placeholder="激活码"><a
                                href="${ctx}/api/register/get_invite_code" class="activationCode color-blue">获取激活码</a></label>
                        <div class="cells-error none">请输入激活码</div>
                    </c:if>
                    <c:if test="${not empty pubUserName}">
                        <p class="cells-block" style="margin-top:.4rem;">该帐号价值 69.9元，由 <span class="color-blue">${pubUserName}</span>为您免费提供。</p>
                    </c:if>
                </div>
...
<script type="text/javascript">
    ...
    function sendMessage() {
        ...
        $.ajax({
            ...
            url: '${ctx}/api/register/get_captcha', //目标地址
