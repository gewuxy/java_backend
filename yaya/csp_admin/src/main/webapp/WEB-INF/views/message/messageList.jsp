...
<table id="contentTable" class="table table-striped table-bordered table-condensed" align="center">
    ...
    <c:if test="${not empty page.dataList}">
        <c:forEach items="${page.dataList}" var="mes">
            <tr>
                <td>${mes.messageTitle}</td>
                <td>${mes.messageContent}</td>
                <td> ${mes.username}</td>
                <td><fmt:formatDate value="${mes.creatTime}" type="both" dateStyle="full"/></td>
                <td><fmt:formatDate value="${mes.updateTime}" type="both" dateStyle="full"/></td>
